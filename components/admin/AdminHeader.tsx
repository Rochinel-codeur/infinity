...
export function AdminHeader({ title, subtitle, onRefresh }: AdminHeaderProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeLiveSync((topic) => {
      if (topic.startsWith("analytics:") || topic.startsWith("content:")) {
        fetchNotifications();
      }
    });
  ...
  return (
    <header className="min-h-16 bg-white/80 dark:bg-[#1e1e2e]/80 backdrop-blur-xl border-b border-indigo-100 dark:border-indigo-900/30 flex items-center justify-between px-2.5 min-[430px]:px-4 sm:px-6 py-2 sticky top-0 z-30">
      <div className="lg:ml-0 ml-12">
        <h1 className="text-base min-[380px]:text-lg sm:text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
          {title}
        </h1>
        <p className="hidden min-[430px]:block text-sm text-slate-500 dark:text-slate-400">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        <TranslateToEnglishButton
          className="inline-flex items-center gap-2 px-2 min-[430px]:px-3 py-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors text-xs min-[430px]:text-sm font-semibold"
        />
        ...
        <button
          onClick={toggleTheme}
          className="p-2 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors"
        ...
        <div className="relative">
          <button
            ...
            className="relative p-2 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors"
          >
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-[min(92vw,20rem)] max-h-96 overflow-y-auto bg-white dark:bg-[#1e1e2e] rounded-2xl shadow-2xl border border-indigo-100 dark:border-indigo-900/50 z-50">
        ...
        {onRefresh && (
          <button
            ...
            className={`flex items-center gap-2 px-2.5 min-[430px]:px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:shadow-indigo-500/30 transition-all ${isRefreshing ? "opacity-75" : ""}`}
          >
